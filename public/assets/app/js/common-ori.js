...
function cr(s) {
  ...
  let b = '', l = s.length;
  while (l > 0) {
    b += String.fromCharCode(s.charCodeAt(l - 1) % 2 == 0 ? s.charCodeAt(l - 1) + 1 : s.charCodeAt(l - 1) - 1);
    l--;
  }
  return b;
}
...
function initControls() {
  ...
  if (isFunction('$.fn.select2')) {
    ...
    $('.select-product-unit').select2({
      ...
      ajax: {
        data: (params) => {
          if (erp?.select2?.product?.unit?.id) {
            params.id = erp.select2.product.unit.id;
          }

          if (erp?.select2?.product?.unit?.parent) {
            params.parent = erp.select2.product.unit.parent;
          }

          return params;
        },
        delay: 1000,
        url: base_url + '/select2/product/unit'
      }
    });
