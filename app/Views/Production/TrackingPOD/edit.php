...
<script>
  ...
  $(document).ready(function() {
    ...
    $('#date').val('<?= dateTimeJS($pod->date) ?>');
    preSelect2('warehouse', '#warehouse', '<?= $pod->warehouse_id ?>').catch(err => console.warn(err));
    preSelect2('product', '#category', '<?= $pod->pod_id ?>').catch(err => console.warn(err));
    $('[name="endclick[]"]').val('<?= $pod->end_click ?>');
    $('[name="rejectmachine[]"]').val('<?= $pod->mc_reject ?>');

    initModalForm({
      form: '#form',
      submit: '#submit',
      url: base_url + '/production/trackingpod/edit/<?= $pod->id ?>'
    });
  });
</script>