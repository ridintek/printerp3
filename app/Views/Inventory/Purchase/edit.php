  ...
  $(document).ready(function() {
    ...
    if (items) {
      for (let item of items) {
        ProductPurchase.table('#table-productpurchase').mode('edit').addItem(item);
      }
    } else {
    ...
    editor.root.innerHTML = `<?= $purchase->note ?>`;
    ...
    $('#date').val('<?= dateTimeJS($purchase->date) ?>');
    preSelect2('biller', '#biller', '<?= $purchase->biller_id ?>').catch(err => console.warn(err));
    preSelect2('expense/category', '#category', '<?= $purchase->category_id ?>').catch(err => console.warn(err));
    preSelect2('supplier', '#supplier', '<?= $purchase->supplier_id ?>').catch(err => console.warn(err));
    preSelect2('user', '#created_by', '<?= $purchase->created_by ?>').catch(err => console.warn(err));
    preSelect2('warehouse', '#warehouse', '<?= $purchase->warehouse_id ?>').catch(err => console.warn(err));

    initModalForm({
      form: '#form',
      submit: '#submit',
      url: base_url + '/inventory/purchase/edit/<?= $purchase->id ?>'
    });
  });
</script>